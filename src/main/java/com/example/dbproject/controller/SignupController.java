...
public class SignupController {
    ...
    @PostMapping(value ="/authenRequest", produces = {MediaType.TEXT_HTML_VALUE})
    ...
    ModelAndView login(@ModelAttribute AuthenticationRequest authenticationRequest ) {
      ...
      if(authentication.isAuthenticated()&&(authentication.getAuthorities().toString().contains("ADMIN"))){
          ...
          log.info("here1 "+mav.hasView());
          return mav;
      }
      else {
          ModelAndView mav = new ModelAndView("failureredirect");
          log.info("here2 "+mav.hasView());
          return mav;
      }
    ...
    public ModelAndView readpath2() throws ExpiredJwtException {
        ModelAndView mav = new ModelAndView("authen");
