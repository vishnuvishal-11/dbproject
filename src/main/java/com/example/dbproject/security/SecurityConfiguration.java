...
//    }
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf().disable().cors().and().authorizeHttpRequests().antMatchers("/home","/authenRequest").permitAll();
        http.csrf().disable().cors().and().authorizeHttpRequests((authz) -> authz .antMatchers(HttpMethod.POST,"/user/*")
                       ...
                       .antMatchers(HttpMethod.GET,"/user/*")
                        ...
                        .hasAnyRole("USER", "CELEBRITY","ADMIN", "SUPER_ADMIN")
                        .regexMatchers( "/auth", "/enter","/swagger-ui/index.html")
                        .permitAll()
                ).exceptionHandling().and().sessionManagement()
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS).and()
                .oauth2Login().defaultSuccessUrl("http://localhost:8090/enter",true)
                 .and()
              //  .formLogin().loginPage("/authen.html").successForwardUrl("http://localhost:8082/swagger-ui/index.html").and()
        ...
        http.csrf().disable().cors().and()
               ...
               .defaultSuccessUrl("http://localhost:8090/enter",true)
                .and()
              //  .formLogin().loginPage("/authen.html").successForwardUrl("http://localhost:8082/swagger-ui/index.html").and()
                .httpBasic(withDefaults());
